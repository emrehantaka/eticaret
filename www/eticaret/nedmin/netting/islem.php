...
if (isset($_POST['logoduzenle'])) {
	...
	if ($_FILES['ayar_logo']['size']>10240) {
		
		echo "Bu dosya boyutu çok büyük";
		Header("Location:../production/genel-ayar.php?durum=buyukdosya");

	}

	$izinli_uzantilar=array('png','gif');
	...
	//echo $_FILES['ayar_logo']["name"];
	 ...
	 if (in_array($ext, $izinli_uzantilar) === false) {
	 	...
	 	Header("Location:../production/genel-ayar.php?durum=formathatalı");
...
if (isset($_POST['bankaekle'])) {
	...
	$kaydet=$db->prepare("INSERT INTO banka SET
		...
		banka_iban=:iban,
		banka_hesapadsoyad=:hesapadsoyad
		");
	...
	$insert=$kaydet->execute(array(
		...
		'hesapadsoyad' => $_POST['banka_hesapadsoyad']
	));
...
if (isset($_POST['bankasiparisekle'])) {
	...
	if ($insert) {
		...
		if ($insert) {
			...
			//Sipariş Detay Kayıtta Başarılıysa Sepeti Boşalt
